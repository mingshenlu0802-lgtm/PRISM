...
> 那份是完整的操作流程，按钮名称跟你在 GitHub / Cloudflare 里看到的英文界面
> 一字不差。这一份讲的是**为什么**——每一步在防什么，以及哪些事其实不用担心。
>
> **要让朋友能看到你的内容，先看 [SHARING.md](SHARING.md)。**
> 接上那个免费数据库之后，权限就由数据库强制执行了，
> 下面 A 部分里那些「静态站挡不住人」的顾虑大半会自动消失。
...
> **接上共享数据库之后（[SHARING.md](SHARING.md)），这一节大半不再适用**——
> 那时权限由数据库强制执行，不在名单上的人一行都读不到。
> 下面说的是**没接后端**的状态。

没接后端时，网站是**纯静态**的：没有服务器，没有数据库。
所有判断（谁能编辑）都在访问者自己的浏览器里跑。这意味着：
...
>
> 网站里**没有任何地方能存 AI 的 API key**——控制端那个能跟 Claude 对话的标签
> 已经整个去掉了，连带 SDK 一起。所以「别人花掉你的额度」这件事，现在不是
> 「防住了」，而是**根本不存在这条路**。
...
| 别人**花掉你的 AI 额度** | 网站里存不了 API key，没有这条路 |
...
> 这是一次性的改动，在 Claude Code 的聊天里说一声就能做
> （「把控制端拆成独立页面，好让 Cloudflare Access 能保护它」）。
...
现在代码里**一个地址都没有**，连哈希都不需要——谁是站长完全来自数据库的成员名单。
GitHub 用户名也从代码里拿掉了，改成你在控制端里填。

发布流程里加了一道自动检查（`npm run privacy`）：
**只要发布产物里出现任何邮箱地址，就不发布。** 这条防线不依赖谁记得住。

> **换站长怎么办？** 代码里不用改任何东西——站长是数据库 `members` 表里的一行。
> 在控制端的成员名单里改，或者在 Supabase 的 SQL Editor 里跑一句：
> ```sql
> update public.members set role = 'member' where email = '旧站长@x.com';
> update public.members set role = 'owner'  where email = '新站长@x.com';
> ```
...
5. 换域名不影响登录——邮箱登录不绑定域名，不用回去改任何设置。
...
登录用的是邮箱链接，没有第三方登录，所以**不会在任何授权窗口里显示你的身份**。

要注意的只有一处：**发登录信的那个发件地址**。
默认是 Supabase 的地址（`noreply@mail.app.supabase.io` 之类），跟你无关，
不会暴露任何东西。以后如果你接自己的发信服务（比如 Resend）并绑定自有域名，
发件地址会变成 `something@你的域名` —— 那时用一个站点相关的名字，
别用 `你的名字@你的域名`。

另外，**成员名单里有所有朋友的邮箱**。它存在你的 Supabase 数据库里，
只有名单上的人能读（站长才能改）。这是别人的个人信息，别导出来乱放。
