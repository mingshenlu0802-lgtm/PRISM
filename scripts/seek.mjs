...
中文媒体和英文媒体的措辞常常完全不同，只搜一种会漏掉一半。`
  ...
  const { text, found } = await askText(system, user, {
     ...
     * 输出预算也要跟着「要几条」走。
     *
     * 原来写死 4000。找 6 条够用，找 18 条就被截断了——而且截断的
     * 不只是正文：`thinking: adaptive` 的思考 token 也算在这个预算里，
     * 搜了十二轮之后思考本身就不便宜。
     *
     * 每条按 500 token 给（六行文字加思考的余量），下限 8000。
     * 这个数字给大了不花钱——按实际输出计费，上限只是「允许写这么多」。
     */
    maxTokens: Math.max(8000, want * 500),
    search: true,
    /*
     * 搜索轮数按**要几条**来，不是一个定数。
     *
